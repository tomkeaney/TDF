...
Stage_list <- list(stage_1_results, stage_2_results, stage_3_results)
                   #, stage_4_results, stage_5_results, stage_6_results, stage_7_results, stage_8_results, stage_9_results, stage_10_results, stage_11_results, stage_12_results, stage_13_results, stage_14_results,stage_15_results, stage_16_results, stage_17_results, stage_18_results, stage_19_results, stage_20_results, stage_21_results)
```
...
Teams <- 
  tibble(`Rider type` = c("General", "General", "General", "General", "Domestique", "Sprinter", "Sub 1", "Sub 2", "12 month PCS points"),
         Zing = c("Pogačar Tadej (1)*", "Cavendish Mark (330)", "Martinez Lenny (32)", "Bilbao Pello (17)", "Almeida Joao (26)", "Pedersen Mads (4)", "Landa Mikel (50)", "Onley Oscar (86)", "10,968"),
         Bergs = c("Vingegaard Jonas (5)", "Gaudu David (94)", "Healy Ben (105)", "Bernal Egan (59)", "Powless Neilson (185)", "Jakobsen Fabio (184)", "Mas Enric (48)", "Fuglsang Jakob (470)", "5,514"),
         Falcs = c("Evenepoel Remco (3)", "Skujins Toms (74)", "Pidcock Tom (28)", "Mohorič Matej (25)", "Van Der Poel Mathieu (9)", "Cort Magnus (219)", "Girmay Biniam (55)", "Thomas Geraint (99)", "8,243"),
         Haz = c("Roglic Primoz (11)", "Buitrago Santiago (75)", "Williams Stephen (36)", "Yates Simon (23)", "Van aert Wout (10)", "De Lie Arnaud (14)", "Van Gils Maxim (20)", "Haig Jack (325)", "8,876"),
         Ben = c("Rodriguez Carlos (16)", "Ayuso Juan (12)", "Ciccone Giulio (133)", "Matthews Michael (69)", "Bettiol Alberto (42)", "Philipsen Jasper (2)", "Carapaz Richard (64)", "Martin Guillaume (47)", "8,768"),
         Willo = c("Yates Adam (7)", "Gee Derek (194)", "Vlasov Aleksandr (8)", "Hindley Jai (45)", "Jorgenson Matteo (22)", "Groenewegen Dylan (34)", "Gall Felix (51)", "Bennett Sam (89)", "8,220"),
         `Free agents` = c("Gregoire Romain (76)", "Demare Arnaud (137)", "Campenaerts Victor (302)",
                           "Lazkano Oier (131)", "Kragh Andersen Soren (160)", "Bauhaus Phil (101)",
                           "NA", "NA", "2,682")) 
...
Results_complete <- 
  ...
  mutate(Directeur = case_when(
    BIB == 81 | BIB == 64 | BIB == 131 | BIB == 21 | BIB == 8 | BIB == 121 ~ "Haz",
    BIB == 1| BIB == 91| BIB == 115| BIB == 32| BIB == 116 | BIB == 185 ~ "Bergs",
    BIB == 71| BIB == 46| BIB == 36| BIB == 66| BIB == 101 | BIB == 201 ~ "Falcs",
    BIB == 11| BIB == 191| BIB == 96| BIB == 61| BIB == 12 | BIB == 45 ~ "Zing",
    BIB == 31 | BIB == 13| BIB == 41| BIB == 26| BIB == 112 | BIB == 106 ~ "Ben",
    BIB == 18| BIB == 135 | BIB == 88 | BIB == 84 | BIB == 3 | BIB == 23 ~ "Willo",
    BIB == 93| BIB == 164 | BIB == 123 | BIB == 155 | BIB == 104 | BIB == 63 ~ "Free agents",
    .default = "Not selected"
  )) %>% 
  # add the jersey wearing points
  mutate(Yellow_wearer_points = case_when(BIB == 181 | BIB == 11 | BIB == 111  ~ "10",
                                          .default = "0"),
         Green_wearer_points = case_when(BIB == 187 | BIB == 202 ~ "5",
                                         .default = "0"),
         White_wearer_points = case_when(BIB == 187 | BIB == 71 ~ "5",
                                         .default = "0"),
         Polka_wearer_points = case_when(BIB == 202 ~ "5",
                                         .default = "0"),
         Domestique_points = case_when(BIB == 3 ~ "1",
                                         .default = "0")) %>% 
  ungroup() %>% 
  ...
  mutate(GC_points = replace_na(GC_points, 0),
         ...
         Jersey_points = Yellow_wearer_points + Green_wearer_points + White_wearer_points + 
           Polka_wearer_points,
         `Green points` = recode(BIB, "187" = 10, "181" = 8, "8" = 6, .default = 0),
         `White points` = recode(BIB, "187" = 10, "126" = 8, "71" = 6, .default = 0),
         `Mountain points` = recode(BIB, "202" = 10, "95" = 6, "187" = 4, .default = 0),
          Final_score = Total_stage_points + GC_points + Total_most_combative_points + 
           ...
           Polka_wearer_points + `Green points` + `Mountain points` + Domestique_points) %>% 
  arrange(-Final_score) %>% 
  select(Rider, BIB, Directeur, GC, Total_stage_points, Total_most_combative_points, GC_points, Jersey_points, Domestique_points, `Green points`, `Mountain points`, Final_score)
...
Rider_rankings %>% 
                select(Rider, `Directeur sportif`, `Provisional score`,
                       `GC points`, `Green points`, `Mountain points`, `Cumulative stage points`, `Cumulative most combative points`, Jersey_points, Domestique_points) %>% 
  my_data_table()
```
