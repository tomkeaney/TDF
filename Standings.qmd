...
Stage_list <- list(stage_1_results, stage_2_results)
                   
                   #, stage_3_results, stage_4_results,
...
Teams <- 
  tibble(`Rider type` = c("General", "General", "General", "General", "Domestique", "Sprinter", "Sub 1", "Sub 2"),
         ...
         Falcs = c("Van Der Poel Mathieu", "Ganna Filippo", "Powless Neilson", "Healy Ben", "Kuss Sepp", "Merlier Tim", "Mohoric Matej", "Theuns Edward"),
         ...
         Bergs = c("Lipowitz Florian", "Mas Enric", "Coquard Bryan", "Buitrago Santiago", "Vlasov Aleksandr", "Philipsen Jasper", "Woods Michael", "Ackermann Pascal"),
...
Results_complete <- 
  ...
  mutate(Yellow_wearer_points = case_when(BIB == 101 | BIB == 106 ~ "10",
                                          .default = "0"),
         Green_wearer_points = case_when(BIB == 101 ~ "5",
                                         .default = "0"),
         White_wearer_points = case_when(BIB == 41 | BIB == 131 ~ "5",
                                         .default = "0"),
         Polka_wearer_points = case_when(BIB == 167 | BIB == 1 ~ "5",
                                         .default = "0"),
         Domestique_points = case_when(BIB == 8 ~ "8", # 2 + 6
                                       BIB == 17 ~ "5", # 1 + 4
                                       BIB == 16 ~ "5", # 1 + 4
                                       BIB == 15 ~ "5", # 1 + 4
                                       BIB == 78 ~ "0", # 
  ...
  mutate(GC_points = replace_na(GC_points, 0),
         ...
         Jersey_points = Yellow_wearer_points + Green_wearer_points + White_wearer_points + 
           Polka_wearer_points,
         `Green points` = recode(BIB, "101" = 10, "41" = 8, "106" = 6, .default = 0),
         `White points` = recode(BIB, "131" = 10, "203" = 8, "21" = 6, .default = 0),
         `Mountain points` = recode(BIB, "1" = 5, "7" = 6, "167" = 4, .default = 0),
